...
import { ArrowLeft, MapPin, Package, Send, MessageCircle, Truck } from "lucide-react"
...
export default function ClientOrderDetailsPage() {
  ...
  useEffect(() => {
    if (order && user && (order.status === "accepted" || order.status === "in_progress")) {
      // Écouter les messages en temps réel seulement si la commande est acceptée
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("=== ENVOI MESSAGE CLIENT ===")
      console.log("Order ID:", orderId)
      console.log("User:", user)
      console.log("Message:", newMessage.trim())

      await sendMessage(orderId, user.uid, user.name, "client", newMessage.trim())
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="text-right">
                  <p className="text-2xl font-bold text-blue-600">{formatPrice(order.price)}</p>
                </div>
              </CardContent>
            </Card>
                  ...
                  <div className="flex items-center space-x-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        Commande acceptée le {formatDate(order.updatedAt)}
                      </p>
                    </div>
                  </div>
                </CardContent>
            ...
            <Card className="h-[600px] flex flex-col">
              <CardHeader>
                <CardTitle className="flex items-center">
                  ...
                  {canChat ? `Chat avec ${order.driverName}` : "Chat"}
                </CardTitle>
                <CardDescription>
                  {canChat
                    ? "Communiquez avec votre chauffeur pour coordonner la livraison"
                    : order.status === "pending"
                      ? "Le chat sera disponible une fois qu'un chauffeur aura accepté votre commande"
                      : order.status === "completed"
                        ? "La commande est terminée, le chat a été fermé"
                        : "Chat non disponible pour cette commande"}
                </CardDescription>
              </CardHeader>
                            ...
                            >
                              ...
                              <p
                                className={`text-xs mt-1 ${
                                  message.senderRole === "client" ? "text-blue-100" : "text-gray-500 dark:text-gray-400"
                                }`}
                              >
